...
void CAi::Init(jint *tbl, jint *src, jint s)
{
	int x = 0;
    pAt = ATbl;
    for (x=0; x<3772; x++) {
			...
			if (c5>=0 && c5<size && c6>=0 && c6<size)
			  if (bd[c1][c2]>0 && bd[c3][c4]>0 &&
			    ...
			    if (bd[c5][c6]==cp) {
			      ...
			      if (c7>=0 && c7<size && c8>=0 && c8<size && Kgame)
...
#if HASH == 1
	HValX[lvl] ^= (bd[x][y]*TableX[x+y*size]);
	HValY[lvl] ^= (bd[x][y]*TableY[x+y*size]);
#endif
		...
		if (bd[c1][c2]>0 && bd[c3][c4]>0 &&
			...
			if (bd[c5][c6]==fr) {
...
#if HASH == 1
				HValX[lvl] ^= (en*TableX[c1+size*c2]);
				HValY[lvl] ^= (en*TableY[c1+size*c2]);
				HValX[lvl] ^= (en*TableX[c3+size*c4]);
				HValY[lvl] ^= (en*TableY[c3+size*c4]);
#endif
				...
				if (bd[c7][c8]==fr && bd[c5][c6]>0) {
					...
					HValX[lvl] ^= (en*TableX[c3+size*c4]);
					HValY[lvl] ^= (en*TableY[c3+size*c4]);
					HValX[lvl] ^= (en*TableX[c5+size*c6]);
					HValY[lvl] ^= (en*TableY[c5+size*c6]);
#endif
				}
			}
		} // if en*2
...
if (dv<4) {
	...
	do { //c1 //look up in table
		...
		if (f0==fr) *(pFk+(x+y*size)*4 +dv)=tys;
